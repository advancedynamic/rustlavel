...
//!
//! # The consent screen needs the application's middleware
//!
//! `/oauth/authorize` reads `Identity` from the request, which the application
//! puts there with `SessionManager` and `Authenticate`, and it posts a form
//! back, which needs the application's `Csrf` middleware in front of it. This
//! crate does not register either: it does not own the application's session
//! store, and silently installing one would be exactly the runtime magic the
//! framework refuses. [`OAuthProvider::register`] warns when the session
//! middleware looks absent at boot rather than at the first login.
...
impl Plugin for OAuthProvider {
    ...
    fn register(self: Box<Self>, setup: &mut Setup<'_>) {
        ...
        rustlavel_core::info!(
            "oauth: authorization server mounted at {} — remember the session and csrf \
...
mod tests {
    ...
    /// A router with the endpoints mounted, and a stand-in for the
    /// application's `auth` middleware that signs everybody in as user 7.
