...
impl QueryBuilder {
    ...
    pub async fn insert(&self, db: &Database, values: &[(&str, Value)]) -> Result<i64> {
        // One path for all three databases: `insert_returning` knows that the
        // key arrives as a row on some and in the acknowledgement on others.
        i64::from_value(&self.insert_returning(db, values, "id").await?)
    }
    ...
    pub async fn insert_returning(
        ...
        db.insert_returning_key(&sql, &params, column).await?.ok_or_else(|| {
            Error::msg(format!(
                "the insert did not report a `{column}`. The column has to be generated by the \
                 database — an auto-incrementing key or a default — for it to have one to report."
            ))
        })
    }
