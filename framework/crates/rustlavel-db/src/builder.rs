...
use crate::{Database, Row, quote_identifier, validate_identifier};
...
impl QueryBuilder {
    ...
    pub fn to_sql(&self) -> Result<(String, Vec<Value>)> {
        ...
        if self.columns.is_empty() {
            ...
            let rendered: Result<Vec<String>> = self.columns.iter().map(|c| column_ref(c)).collect();
        ...
        if !self.conditions.is_empty() {
            sql.push_str(" where ");
            sql.push_str(&render_conditions(&self.conditions, &mut params)?);
        }

        if !self.group.is_empty() {
            let rendered: Result<Vec<String>> = self.group.iter().map(|c| column_ref(c)).collect();
        ...
        if !self.order.is_empty() {
            let rendered: Result<Vec<String>> = self
                .order
                .iter()
                .map(|(column, direction)| Ok(format!("{} {}", column_ref(column)?, direction.as_sql())))
                .collect();
    ...
    pub fn to_count_sql(&self) -> Result<(String, Vec<Value>)> {
    ...
    fn to_insert_sql(&self, rows: &[Vec<(String, Value)>], returning: Option<&str>) -> Result<(String, Vec<Value>)> {
        ...
        let rendered: Result<Vec<String>> =
            columns.iter().map(|c| validate_identifier(c).map(|_| format!("\"{c}\""))).collect();
    ...
    fn to_update_sql(&self, values: &[(String, Value)]) -> Result<(String, Vec<Value>)> {
        if values.is_empty() {
    ...
    fn to_delete_sql(&self, allow_all: bool) -> Result<(String, Vec<Value>)> {
        let mut params = Vec::new();
    ...
    pub async fn first(&self, db: &Database) -> Result<Option<Row>> {
        let (sql, params) = self.clone().limit(1).to_sql()?;
    ...
    pub async fn insert(&self, db: &Database, values: &[(&str, Value)]) -> Result<i64> {
        ...
        let (sql, params) = self.to_insert_sql(std::slice::from_ref(&row), Some("id"))?;
    ...
    pub async fn insert_returning(
        ...
        let row: Vec<(String, Value)> =
            values.iter().map(|(name, value)| ((*name).to_string(), value.clone())).collect();
        let (sql, params) = self.to_insert_sql(std::slice::from_ref(&row), Some(column))?;
    ...
    pub async fn insert_without_id(&self, db: &Database, values: &[(&str, Value)]) -> Result<u64> {
        let row: Vec<(String, Value)> =
            values.iter().map(|(name, value)| ((*name).to_string(), value.clone())).collect();
        let (sql, params) = self.to_insert_sql(std::slice::from_ref(&row), None)?;
    ...
    pub async fn insert_many(&self, db: &Database, rows: &[Vec<(String, Value)>]) -> Result<u64> {
        ...
        let (sql, params) = self.to_insert_sql(rows, None)?;
        db.execute(&sql, &params).await
    }
    ...
    for (index, condition) in conditions.iter().enumerate() {
        ...
        match condition {
            Condition::Comparison { column, operator, value, .. } => {
                ...
                params.push(value.clone());
                out.push_str(&format!("{} {operator} ${}", column_ref(column)?, params.len()));
            }
            ...
            Condition::Group { conditions, .. } => {
                out.push_str(&format!("({})", render_conditions(conditions, params)?));
            }
        }
    }
...
fn column_ref(column: &str) -> Result<String> {
    if column == "*" {
        return Ok("*".to_string());
    }
    ...
    if let Some(rest) = column.strip_prefix("count(*) as ") {
        ...
        return Ok(format!("count(*) as \"{rest}\""));
    }
    ...
    for prefix in ["count", "sum", "avg", "min", "max"] {
        ...
        {
            let rendered = if inner == "*" { "*".to_string() } else { quote_identifier(inner)? };
...
mod tests {
    ...
    fn an_injected_identifier_is_rejected() {
        let error = QueryBuilder::new("users").filter("name; drop table users", 1).to_sql().unwrap_err();
        assert!(error.to_string().contains("not a valid SQL identifier"));

        assert!(QueryBuilder::new("users; drop table users").to_sql().is_err());
    }

    #[test]
    fn an_unknown_operator_is_rejected() {
        let error = QueryBuilder::new("users").filter_op("id", "; drop", 1).to_sql().unwrap_err();
    ...
    fn builds_an_insert_with_a_returning_clause() {
        ...
        let (sql, params) =
            QueryBuilder::new("users").to_insert_sql(std::slice::from_ref(&row), Some("id")).unwrap();
    ...
    fn a_bulk_insert_numbers_every_row() {
        ...
        let (sql, params) = QueryBuilder::new("users").to_insert_sql(&rows, None).unwrap();
    ...
    fn update_and_delete_refuse_to_touch_every_row_by_accident() {
        ...
        let error = QueryBuilder::new("users").to_update_sql(&values).unwrap_err();
        assert!(error.to_string().contains("refusing to update every row"));

        let error = QueryBuilder::new("users").to_delete_sql(false).unwrap_err();
        assert!(error.to_string().contains("refusing to delete every row"));

        // The explicit forms are allowed.
        assert!(QueryBuilder::new("users").to_delete_sql(true).is_ok());
        assert!(
            QueryBuilder::new("users")
                .filter("id", 1)
                .to_update_sql(&values)
                .is_ok()
        );
    }
    ...
    fn an_update_numbers_values_before_conditions() {
        ...
        let (sql, params) =
            QueryBuilder::new("users").filter("id", 7).to_update_sql(&values).unwrap();
