...
8. **Credentials can rotate under a live pool.** `rustlavel-db/src/credentials.rs` holds a shared username/password plus a generation counter; every driver calls `config.resolved()` on the way into a connect, and the pool retires connections opened under a superseded generation. Measured, not assumed: an already-open PostgreSQL connection keeps working after its role is dropped, and only a *new* connection is refused — so rotation never interrupts work in flight. `rustlavel-db` must never depend on `rustlavel-vault`; the application does the wiring.
