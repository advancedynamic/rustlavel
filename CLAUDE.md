...
8. **The database layer is split in two.** A `Dialect` (in `rustlavel-db/src/dialect.rs`) holds what SQL databases disagree about; a `Driver` (in `driver.rs`) holds the wire protocol. Everything above that line — query builder, schema builder, migrator, ORM — is written once and must stay database-agnostic. The PostgreSQL driver is the reference implementation; a new driver should read as though the same person wrote it.
9. Verify with `cargo test --workspace --all-features` and `cargo clippy --workspace --all-features --all-targets` — zero warnings — from inside `framework/`, before claiming anything is finished.
