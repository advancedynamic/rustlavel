#!/usr/bin/env bash
#
# Print the environment the integration suites read, as KEY=value lines.
#
...
#   ./docker/env.sh search >> "$GITHUB_ENV"
#
...
if want db; then
  ...
  echo "PG_TLS_URL=postgres://rustlavel:secret@localhost:55432/rustlavel_test"
  echo "PG_TLS_CA=$certs/ca.crt"
  ...
  echo "MYSQL_TLS_URL=mysql://rustlavel:secret@127.0.0.1:33306/rustlavel_test"
  echo "MYSQL_TLS_CA=$certs/mysql-ca.pem"
  ...
  echo "REVOCATION_PG_URL=postgres://postgres:rootpass@127.0.0.1:55433/appdb?sslmode=disable"
  ...
  echo "REVOCATION_MYSQL_URL=mysql://root:secret@127.0.0.1:33306/appdb"
  echo "REVOCATION_MSSQL_URL=sqlserver://sa:Rustlavel!2026@127.0.0.1:51433/master"
  ...
  echo "ROTATION_DB_URL=postgres://127.0.0.1:55433/appdb?sslmode=disable"
  echo "VAULT_ADDR=http://127.0.0.1:18200"
...
if want ldap; then
  ...
  echo "LDAP_TEST_URL=ldap://127.0.0.1:3389"
  ...
  echo "LDAPS_TEST_URL=ldaps://localhost:3636"
  echo "LDAP_STARTTLS_TEST_URL=ldap://localhost:3390"
fi
...
if want search; then
  ...
  echo "ELASTICSEARCH_URL=http://127.0.0.1:9200"
  echo "ELASTICSEARCH_USER=elastic"
  echo "ELASTICSEARCH_PASSWORD=rustlavel-secret"
fi
...
if want otel; then
  ...
  # was meant.
  echo "OTEL_TEST_ENDPOINT=http://127.0.0.1:4318"
  echo "OTEL_TEST_CONTAINER=rustlavel-otel"
fi
...
if want secrets; then
  ...
  echo "VAULT_TEST_DATABASE_ROLE=app-readwrite"
  echo "VAULT_TEST_DATABASE_HOST=127.0.0.1:55433/appdb"
fi
